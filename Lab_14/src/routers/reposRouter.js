...
reposRouter.put(
    ...
    async (req, res, next) => {
        try{
            ...
            console.log("AUTHORID: " + repo.author.id + " USERID: " + req.user.id + " ROLE: " + req.user.role);
            if (req.user.id != repo.author.id && req.user.role != 'Admin')
                throw new Error("Охренел?");
    ...
    async (req, res, next) => {
        try {
            ...
            console.log("COMMITREPOID: " + commit.repoId);
